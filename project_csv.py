...
def esporta_fotografia_csv(fotografia: Mapping[str, Any]) -> bytes:
    ...
    fotografia_completa = {
        ...
        "immagini": immagini,
        # È una sintesi deterministica del progetto, mai una seconda copia
        # del manoscritto. Viene conservata per guidare la stesura successiva.
        "memoria_editoriale": dict(fotografia.get("memoria_editoriale", {}) or {}),
    ...
    writer.writeheader()
    writer.writerow({"tipo": "formato", "chiave": "scrittore_site", "valore": "4"})
...
def importa_fotografia_csv(dati_grezzi: bytes, campi_sidebar_validi: Iterable[str]) -> dict[str, Any]:
    ...
    snapshot: dict[str, Any] = {
        "sidebar": {}, "indice_raw": "", "indice_backup": "", "contenuti": {}, "fonti": {}, "immagini_capitoli": {}, "memoria_editoriale": {}, "controlli": {}
    }
    formato_valido = False
    validi = set(campi_sidebar_validi)
    for riga in righe:
        tipo, chiave, valore = riga.get("tipo", ""), riga.get("chiave", ""), riga.get("valore", "")
        if tipo == "formato" and chiave == "scrittore_site" and valore in {"1", "2", "3", "4"}:
            formato_valido = True
